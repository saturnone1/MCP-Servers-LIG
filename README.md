...
The build writes the normal-user `Setup.exe` to `installer\output` and separates the managed-deployment MSI under `installer\output\admin`. Give normal users only the self-elevating `Setup.exe`; it requests elevation before extracting and invoking its embedded MSI, avoiding non-elevated Windows Installer execution and the resulting 2502/2503 errors. Reserve the MSI for elevated administrator terminals and managed software deployment. Users do not need the `mcp-bundle` directory, ZIP archive, WiX, or a separate .NET/ASP.NET Core runtime installer. The installer includes all 19 MCP servers, the shared runtime, a self-contained Manager with the product icon embedded, Start Menu and desktop shortcuts, upgrade, repair, and uninstall support. The shortcuts launch `McpManager.exe` directly instead of a `.cmd` wrapper so the product icon is used on the taskbar. Installation is per-machine under Program Files; writable settings, logs, and PID files stay under `%LOCALAPPDATA%\LIG AI MCP\.mcp-manager`. External applications such as Git, Docker, kubectl, MATLAB, AutoCAD, SolidWorks, and Rhapsody are still required when their integrations are used. The setup files are currently unsigned, so production distribution should apply the organization's code-signing certificate.
