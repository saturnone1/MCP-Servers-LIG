...
일반 사용자에게는 생성된 `LIG-AI-MCP-Setup-<version>-win-x64.exe` 파일 하나만 배포합니다. Setup은 설치·업그레이드·복구·제거 시 필요한 UAC 상승을 처리합니다. `.msi`는 이미 상승된 관리자 터미널, 그룹 정책 또는 소프트웨어 배포 시스템에서 직접 배포할 때만 사용합니다. `mcp-bundle` 폴더, ZIP, WiX 도구 또는 별도의 .NET/ASP.NET Core 런타임 설치 파일은 필요하지 않습니다.
